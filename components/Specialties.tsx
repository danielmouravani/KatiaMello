    ...
    <section id="specialties" className="py-24 bg-slate-50">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-brand-700 font-bold tracking-wider uppercase text-sm mb-2">Tratamentos</h2>
          ...
          {SPECIALTIES.map((spec, index) => (
            <div key={index} className="group bg-white rounded-2xl p-8 shadow-sm hover:shadow-2xl hover:shadow-brand-500/10 transition-all duration-300 border border-slate-100 hover:border-brand-200 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="w-14 h-14 bg-brand-50 rounded-xl flex items-center justify-center text-brand-700 mb-6 group-hover:scale-110 transition-transform duration-300">
                  <spec.icon size={28} />
                </div>
                ...
                <button 
                  ...
                  className="inline-flex items-center text-sm font-bold text-brand-700 hover:text-brand-800 group-hover:gap-2 transition-all cursor-pointer focus:outline-none"
                  aria-label={`Saiba mais sobre ${spec.title}`}
                >
